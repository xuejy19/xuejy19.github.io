...
### 等距特征映射(ISOMAP) 
#### 流形学习 
流形学习是一类借鉴了拓扑流形概念的降维方法。“流形”是在局部与欧式空间同胚的空间，换言之，它在局部具有欧式空间的性质，可以用欧式距离来进行距离计算。当低维流形嵌入到高维空间时，则数据样本尽管在高维空间中看起来非常复杂，但在局部仍然具有欧式空间的性质，因此，可以很容易地在局部建立降维映射关系，然后再将局部映射关系推广到全局。下图中作图就是一个二维流形嵌入到了三维空间，若利用流形学习的算法将其降到二维，则如右图所示。
![流形数据](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/manifold.png)
...
$$
    x'_i = w_{ij} x'_j + w_{ik} x'_k + w_{il} x'_l
$$
具体算法思想如下图所示，可以看到所有降维算法的目的都是期望降维后的数据能够与原始空间中的数据尽可能“相似”，选用不同的度量“相似性”的方法也就产生了不同的降维算法。
![Local linear embedding](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/lle1.png)
...
##### 权重矩阵求解 
首先假设原始空间中共有$n$个样本$(x_1, x_2, \dots, x_n)$, 假设对于样本点$x_i$，其近邻点分别为$x_{i1}, x_{i2}, \dots, x_{ik}$, 则我们期望找到这样一组权重：
$$
    w_i = [w_{i1}, w_{i2}, \dots, w_{ik}]^T
$$
使得$||x_i - \sum_{j=1}^k w_{ij} x_{ij} ||^2$尽可能小，这样目标函数就可以写做:
$$
    \begin{aligned}
...
$$
    L = \sum_{i=1}^n w_i^T S_i w_i  - \sum_{i=1}^n \lambda_i(w_i^T \mathbf{1} - 1)
$$
对$w_i$求偏导可得:
$$
    \frac{\partial L}{\partial w_i} = 2 S_i w_i - \lambda_i \mathbf{1} = 0
$$
...
$$
    W = [w'_1, w'_2, \dots, w'_n]_{n \times n}
$$
...
$$
    L = trace(Z M Z^T) - \lambda (Z Z^T - nI) 
$$
若将变量矩阵$Z$以行向量形式来表示：
$$
    Z = [p_1^T;p_2^T,p_{d'}^T] 
$$
那么Lagrange函数就可以写做：
$$
    L = \sum_{i=1}^{d'} p_i^T M p_i - \sum_{i=1}^{d'} \lambda_i (p_i^T p_i - 1)
$$
对$p_i$求导可得:
$$
    \frac{\partial L}{\partial p_i} = 2M p_i = 2\lambda_i p_i
$$
...
### 实验部分 
#### PCA 
将MNIST手写数字数据集应用PCA从28*28维降到1维和2维的效果图如下:
![PCA:1维](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/pca1.png)
![PCA:2维](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/pca2.png)

#### MDS
给若干城市之间的乘车时间信息作为相似度矩阵，然后还原这些城市的坐标。
![MDS](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/mds.png)

#### ISOMAP 
三维空间中的流形数据降到二维
![流形数据](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/Z3.png) 
![ISOMAP降维](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/Z20.png)
#### LLE 
三维空间中的流形数据降到二维
![流形数据](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/LLE.png)
![LLE降维](https://raw.githubusercontent.com/xuejy19/xuejy19.github.io/source/Img/k10.png)